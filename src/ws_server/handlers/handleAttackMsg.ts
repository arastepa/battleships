...
export function handleAttackMsg(
  ...
  const allKilled = enemyShips.every(
    (ship) => (ship.hits || 0) === ship.length,
  );
  if (allKilled) {
    playerIds.forEach((pid) => {
      const player = game.players[pid];
